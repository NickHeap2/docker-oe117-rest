...
LOG_FILES="/usr/local/tomcat/logs/catalina.out /usr/wrk/${WEBSERVICE_NAME}.rest.log"
touch ${LOG_FILES}
...
#sed -i "s|rest1|${WEBSERVICE_NAME}|g" /usr/local/tomcat/webapps/rest/WEB-INF/web.xml
# replace values in ubroker.properties file
#sed -i "s|rest1|${WEBSERVICE_NAME}|g;\
