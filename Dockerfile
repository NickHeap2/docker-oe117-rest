...
FROM oe117-setup:0.1 AS rest_install
...
COPY --from=rest_install /usr/dlc/ /usr/dlc/

# set defaults
ENV DLC=/usr/dlc \
    WEBSERVICE_NAME=rest1 \
...
RUN cp -r webapps.dist/manager/ webapps/manager && \
    ...
    cp -r /usr/dlc/servlets/rest/* /usr/local/tomcat/webapps/
...
RUN sed -i "s|<Valve|<!--<Valve|g; s|1\" />|1\" /> -->|g" webapps/manager/META-INF/context.xml \
